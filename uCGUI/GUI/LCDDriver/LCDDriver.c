...
*/
//#define SETPIXEL(x, y, c) ili9320_SetPixelIndex(x, y, c)
#define GETPIXEL(x, y)    ili9320_GetPixelIndex(x,y)
...
static int _CheckBound(unsigned int c) {
  ...
  if (c > NumColors) {
    GUI_DEBUG_ERROROUT("LCDDriver::SETPIXEL: parameters out of bounds");
    return 1;
  }
  return 0;
}
...
static void _XorPixel(int x, int y) {
  ...
  ili9320_SetPixelIndex(x, y, LCD_NUM_COLORS-1-Index);
}
